# Polygon OMS Studio

Interactive pitch studio for Polygon Open Money Stack.

## Run

```bash
npm install
npm run dev
```

Open `http://localhost:3000`.

## What It Covers

- Launch New mode for founders building a fintech, payment, remittance, payroll, agent-payment, gaming, or CDK product from zero.
- Modernize Existing mode for neobanks, PSPs, payroll platforms, marketplaces, and remittance apps consolidating fragmented vendors.
- OMS module map for wallet infrastructure, crosschain interoperability, stablecoin orchestration, blockchain integration, cash ramps, on/off-ramp, cross-border payments, CDK, and compliance/security.
- Competitor research packs with pricing evidence labels: published, public signal, or custom.
- Scenario savings model, architecture canvas, compliance controls, migration playbook, and PMM export.

## API Routes

- `GET /api/providers`
- `GET /api/sources`
- `POST /api/cost-model`
- `POST /api/recommend/new`
- `POST /api/recommend/migration`
- `POST /api/export-pitch`

## Pricing Caveat

Polygon OMS platform pricing is early-access/custom. The app uses public competitor pricing signals, public Polygon network cost signals, and scenario modeling rather than presenting exact OMS quotes as public.
